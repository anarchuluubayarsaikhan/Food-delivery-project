...
const TeacherWebThirdLayout: React.FC = () => {
  ...
  const freeVideos = [
    { id: 1, src: '/vid1.mp4', alt: 'Video 1' },
    { id: 2, src: '/vid2.mp4', alt: 'Video 2' },
    { id: 3, src: '/vid3.mp4', alt: 'Video 3' },
  ];

  const newVideos = [
    { id: 1, src: '/vid4.mp4', alt: 'Video 1' },
    { id: 2, src: '/vid5.mp4', alt: 'Video 2' },
    { id: 3, src: '/vid6.mp4', alt: 'Video 3' },
  ];

  const freeTexts = [{ title: 'Video 1' }, { title: 'Video 2' }, { title: 'Video 3' }];

  const newTexts = [{ title: 'Video 1' }, { title: 'Video 2' }, { title: 'Video 3' }];
        ...
        <div className="flex gap-8 justify-center">
          {freeVideos.map((video, index) => (
            <div key={video.id} className="relative">
              <video id={`free-${index}`} className="rounded-md shadow-sm w-[450px]" muted loop aria-label={video.alt}>
              ...
              >
                {isPlaying[`free-${index}`] ? <Pause className="text-green-600 w-6 h-6" /> : <PlayIcon className="text-green-600 w-6 h-6" />}
              </div>
            </div>
          ))}
        </div>
      ...
      <div>
        ...
        <div className="flex gap-6 justify-center">
          {newVideos.map((video, index) => (
            <div key={video.id} className="relative">
              <video id={`new-${index}`} className="rounded-md shadow-sm w-[450px]" muted loop aria-label={video.alt}>
                <source src={video.src} type="video/mp4" />
              </video>
              ...
              <div
                ...
                className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 bg-gray-800 p-3 rounded-full shadow-lg cursor-pointer"
              >
                {isPlaying[`new-${index}`] ? <Pause className="text-green-600 w-6 h-6" /> : <PlayIcon className="text-green-600 w-6 h-6" />}
              </div>
            </div>
          ))}
        </div>
