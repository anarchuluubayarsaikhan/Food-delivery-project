  ...
  return (
    <div className="bg-zinc-50 flex pt-36">
      <div className="pl-40 h-[790px] relative">
        <video ref={videoRef} className="rounded-3xl object-cover w-[90%] h-[78%]" autoPlay muted loop aria-label="Educational video">
        ...
        >
          {isPlaying ? <Pause className="text-green-500 w-8 h-8" /> : <Play className="text-green-500 w-8 h-8" />}
        </div>
      </div>
      ...
      <div className="text-emerald-950 font-deacon">
        <div ref={textRef} className={`text-8xl font-black flex flex-col text-green-600 ${isVisible ? 'fade-in-up' : ''}`}>
          <h1 className="text-8xl font-black text-green-600">DOING GOOD,</h1>
          <h1 className="myclass text-8xl font-black text-white">TOGETHER.</h1>
        </div>
        <div className="mt-14 flex flex-col gap-20">
          <div className="text-2xl w-[749px]">
            “Running a startup apparel business can be difficult; and managing cashflow has always been one of our most important challenges. Kikin helped us navigate this with their quick and easy
            application process and rapid turn around time. I've already recommended them to a number of other businesses we know!”
          </div>
