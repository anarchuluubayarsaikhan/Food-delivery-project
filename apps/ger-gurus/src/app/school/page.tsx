...
export default function Page() {
  ...
  function Editable() {
    if (currentUser?._id === currentSchool?.ownerId) {
      ...
      {domain && !loading && (
        <>
                  ...
                  {isDropdownOpen && (
                    <div className="absolute top-full mt-2 right-0 w-48 bg-white border border-gray-300 rounded shadow-lg">
                      <ul className="flex flex-col">
                        <li className="px-2 border-b border-slate-300">{currentUser?.email}</li>
                        {owner &&
                          (editable ? (
                            <li onClick={edit} className="p-2 hover:bg-gray-100 cursor-pointer">
                              Хадгалах
                            </li>
                          ) : (
                            <li
                              onClick={() => {
                                setEditable(true), setDropdownOpen(false);
                              }}
          ...
          <div className="relative">
            ...
            {/* Floating Bubbles */}
            <div className="absolute top-24 left-0 w-full h-[500px] mt-[-145px] overflow-hidden" style={{ pointerEvents: 'none' }}>
            ...
            <div className="flex flex-col items-center mx-auto mt-32">
              <div className="text-9xl font-black text-green-600">
                <motion.h1
                  ...
                  className={`text-9xl font-black text-green-600 hero_h1-green text-center ${editable && 'border-solid border-black border'}`}
              ...
              <div className="w-[671px] h-16 mx-auto text-center mt-10">
                <motion.p
                  contentEditable={editable}
                  ref={refDesc}
                  suppressContentEditableWarning={true}
                  className={`text-base mt-6 ${editable && 'border-solid border-black border'}`}
          ...
          <FlowText flow={currentSchool?.flow} />
          <TeacherWebSecondLayout intro={currentSchool?.introduce || 'Default intro'} desc={currentSchool?.introduceDesc} />
          <LogoGallery />
          <FooterOfSchool />
          {editable && (
            <div className="fixed bottom-5 right-5 flex gap-2">
              <Button onClick={cancelEdit}>Цуцлах</Button>
              <Button onClick={edit}>Хадгалах</Button>
            </div>
          )}
        </>
      )}
