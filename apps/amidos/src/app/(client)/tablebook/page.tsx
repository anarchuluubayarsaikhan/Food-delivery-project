...
export default function TableBook() {
  ...
  const renderButtons = (items: Array<{ value: string }>, setter: (value: string) => void, selectedValue: string | null) =>
    items.map((item, index) => (
      <Button key={index} variant={'amidos2'} className={`text-base font-semibold ${selectedValue === item.value ? 'bg-[#52071B] text-white' : ''}`} onClick={() => setter(item.value)}>
        {item.value}
      </Button>
    ));
  useEffect(() => {
      ...
      <div className="flex justify-between items-center">
        <div className="flex gap-8 p-3">
          <div className="flex flex-col gap-4">
            ...
            <DatePicker selected={new Date(day)} onSelect={(val) => setDay(val ? new Date(val).toISOString() : new Date().toISOString())} className="rounded-md border p-2" />
          </div>
          <div className="flex flex-col gap-4">
          ...
          <div className="flex flex-col gap-4">
            ...
            <Input placeholder="Хүний тоо" type="number" min="1" value={reservedSeat || ''} onChange={(e) => setReservedSeat(Number(e.target.value) || null)} className="p-5" />
          </div>
        </div>
        <Button
